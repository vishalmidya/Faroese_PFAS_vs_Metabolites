...
conf_met <- conf_met[conf_met$Method.RT == "HILIC+",]
...
common_chems_hilic <- intersect(unique(stage4_hilic$Formula), conf_met$Elemental.Composition)
length(common_chems_hilic)
# [1] 174

ft <- conf_met[conf_met$Elemental.Composition %in% common_chems_hilic,c("KEGG.ID","HMDB.ID","Elemental.Composition")]
# [1] 228
...
stage4_hilic <- stage4_hilic[which(stage4_hilic$Formula %in% common_chems_hilic),]
stage4_hilic <- merge(stage4_hilic, ft, by = "Formula") 
# 5137

### Match by adduct and common metabolites

rnames <- NA_character_
for(i in 1:nrow(stage4_hilic)){
  if(length(intersect(stage4_hilic$Adduct[i], unique(conf_met$adduct[conf_met$Elemental.Composition %in% stage4_hilic$Formula[i]]))) != 0){
    rnames <- c(rnames,as.character(i)) 
  }
}

stage4_hilic <- stage4_hilic[rownames(stage4_hilic) %in% rnames,]
stage4_hilic$KEGG.ID <- ifelse(stage4_hilic$KEGG.ID == "-",NA_character_,stage4_hilic$KEGG.ID)
stage4_hilic$HMDB.ID <- ifelse(stage4_hilic$HMDB.ID == "-",NA_character_,stage4_hilic$HMDB.ID)

dim(stage4_hilic)
# [1] 1136   16

head(stage4_hilic)
length(unique(stage4_hilic$KEGG.ID))
# [1] 85

# Characterize metabolites (with KEGG ID) from median_sum_feature_hilic using confirmed metabolites

median_sum_feature_hilic$KEGG <- rep(NA_character_, nrow(median_sum_feature_hilic))
median_sum_feature_hilic$seq <- paste0("chem_",seq(1, nrow(median_sum_feature_hilic)))


for(i in 1:nrow(median_sum_feature_hilic)){
  
  rrt <- stage4_hilic$KEGG.ID[which(abs(median_sum_feature_hilic$mz[i] - stage4_hilic$mz) < 5)]  ###### criteria
  a <- unique(rrt[!is.na(rrt)])
  b <- unique(stage4_hilic$KEGG.ID[stage4_hilic$KEGG.ID %in% a][abs(median_sum_feature_hilic$time[i] - stage4_hilic$time[stage4_hilic$KEGG.ID %in% a]) < 30])
  
  if(length(b)!= 0){
    
    score <- max(stage4_hilic$Annotation.confidence.score[stage4_hilic$KEGG.ID %in% b])
    dft <- stage4_hilic[stage4_hilic$KEGG.ID %in% b,c("KEGG.ID","Annotation.confidence.score")]
    median_sum_feature_hilic$KEGG[i] = knitr::combine_words(unique(dft$KEGG.ID[dft$Annotation.confidence.score == score]), and = "", sep = "/")
...
length(unique(median_sum_feature_hilic$KEGG)) 
# 109


length(median_sum_feature_hilic[is.na(median_sum_feature_hilic$KEGG) == FALSE, ]$KEGG)
# 1991



# Data standardization?: for each participant??

median_sum_feature_hilic <- median_sum_feature_hilic[!is.na(median_sum_feature_hilic$KEGG),] # get rid of rows with missing KEGG ID
median_sum_feature_hilic[,valid_colnames] <- log(median_sum_feature_hilic[,valid_colnames]+1, base = 2)
median_sum_feature_hilic[,valid_colnames] <- scale(median_sum_feature_hilic[,valid_colnames])

rownames(median_sum_feature_hilic) <- as.character(seq(1,nrow(median_sum_feature_hilic)))
...
median_sum_feature_hilic$Annotation.confidence.score <- rep(NA_real_, nrow(median_sum_feature_hilic))
median_sum_feature_hilic$chem_name <- rep(NA_character_, nrow(median_sum_feature_hilic))

for(i in 1:nrow(median_sum_feature_hilic)){
  ffg <- unique(strsplit(median_sum_feature_hilic$KEGG[i],"/")[[1]])
  median_sum_feature_hilic$Annotation.confidence.score[i] <- paste(unique(stage4_hilic$Annotation.confidence.score[stage4_hilic$KEGG.ID %in% ffg]), collapse = ";")
  median_sum_feature_hilic$chem_name[i] <- paste(unique(stage4_hilic$Name[stage4_hilic$KEGG.ID %in% ffg]), collapse = ";")
...
# 
# u_chems <- unique(median_sum_feature_hilic$KEGG)
# r_names <- NA_character_
# for(i in 1:length(u_chems)){
#   r_names <- c(r_names,names(which.max(apply(median_sum_feature_hilic[median_sum_feature_hilic$KEGG %in% u_chems[i],valid_colnames], 1,
#                                              function(x){sd(x, na.rm = T)/mean(x, na.rm = T)}))))
# }
# r_names <- r_names[-1]
# dim(median_sum_feature_hilic[r_names,])
# median_sum_feature_hilic <- median_sum_feature_hilic[r_names,]

data_hilic <- median_sum_feature_hilic[is.na(median_sum_feature_hilic$KEGG)==F,c(valid_colnames,"mz","time","KEGG","seq","Annotation.confidence.score","chem_name")]
dim(data_hilic) 
# [1] 1991 1242
...
# unique(median_sum_feature_hilic$KEGG) 


#list of adduct
unique(stage4_hilic$Adduct)

# "M+H"     "M+Na"    "M+2Na-H"

toy <- median_sum_feature_hilic
joy <- toy[1,]
for(i in 1: nrow(toy)){
...
data_hilic$Class <- toy$most_freq_class
data_hilic$Subclass_1 <- toy$most_freq_Subclass_1
data_hilic$Subclass_2 <- toy$most_freq_Subclass_2

data_hilic[rownames(data_hilic)[data_hilic$Subclass_1 == "Amino acids" & data_hilic$Class == "Organic acids"],"Subclass_1"] = "none"
data_hilic[rownames(data_hilic)[data_hilic$Subclass_1 == "Vitamins" & data_hilic$Class == "Organic acids"],"Subclass_1"] = "none"
data_hilic$Met_id <- paste0("Met",seq(1:nrow(data_hilic)))

# data_hilic[rownames(data_hilic)[data_hilic$Subclass_1 == "Fatty acyls" & data_hilic$Class == "Peptides"],"Subclass_1"] = "none"
# data_hilic[rownames(data_hilic)[data_hilic$Subclass_2 == "Amino sugars" & data_hilic$Subclass_1 == "Carboxylic acids"],"Subclass_2"] = "none"
# data_hilic[rownames(data_hilic)[data_hilic$Subclass_2 == "Fatty Acids and Conjugates" & data_hilic$Subclass_1 == "none"],"Subclass_2"] = "none"
# data_hilic[rownames(data_hilic)[data_hilic$Subclass_1 == "none"],"Subclass_2"] = "none"
# table(data_hilic$Class)
# table(data_hilic$Subclass_1)
# table(data_hilic$Subclass_2)
...
# data_hilic$Class[order(data_hilic$Class,data_hilic$Met_id)]
# data_hilic$Met_id[order(data_hilic$Class,data_hilic$Met_id)]

t_data_hilic <- data.table::transpose(data_hilic)
# get row and colnames in order
colnames(t_data_hilic) <- data_hilic$Met_id
t_data_hilic$id_Year <- colnames(data_hilic)
dim(t_data_hilic)


# View(t_data_hilic)
# t_data_hilic[,c("chem_1","chem_2","id_Year")]

t_data_hilic$id <- rep(NA_character_,nrow(t_data_hilic))
t_data_hilic$Year <- rep(NA_character_,nrow(t_data_hilic))

for(i in 1:nrow(t_data_hilic)){
  t_data_hilic$id[i] <- strsplit(t_data_hilic$id_Year[i],"_")[[1]][1]
  t_data_hilic$Year[i] <- as.numeric(knitr::combine_words(strsplit(strsplit(t_data_hilic$id_Year[i],"_")[[1]][2],"y")[[1]][1],and = "", sep =""))
}


t_data_hilic <- t_data_hilic[,c(data_hilic$Met_id[order(data_hilic$Class,data_hilic$Met_id)], "id","Year")]
...
## Subset for only 125 ids (who have value at year7)

# length((merged_omics %>% filter(Year == 7))$id) #125
# length((merged_omics %>% filter(Year == 14))$id) #125
# length((merged_omics %>% filter(Year == 22))$id) #493
# length((merged_omics %>% filter(Year == 28))$id) #493
...
merged_omics[,(colnames(merged_omics) %in% c(data_hilic$Met_id[order(data_hilic$Class,data_hilic$Met_id)]))] <- (apply(merged_omics[,(colnames(merged_omics) %in% c(data_hilic$Met_id[order(data_hilic$Class,data_hilic$Met_id)]))],2,as.numeric))


merged_omics$Carbohydrates <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Carbohydrates"])])
merged_omics$Hormones <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Hormones and transmitters"])])
merged_omics$Lipids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Lipids"])])
merged_omics$Nucleic_acids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Nucleic acids"])])
merged_omics$Organic_acids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Organic acids"])])
merged_omics$Other <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Other"])])
merged_omics$Peptides <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Class == "Peptides"])])
...
dim(merged_omics[merged_omics$Year == 7,(colnames(merged_omics) %in% c(data_hilic$Met_id[order(data_hilic$Class,data_hilic$Met_id)]))])
...
# write.csv(merged_omics, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/New_faroese/HILIC/merged_omics_hilic.csv", row.names = F)
...
# merged_omics$Monosaccharides <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_1 == "Monosaccharides"])])
# merged_omics$Disaccharides <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Disaccharides"])])
# merged_omics$Aldoses <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Aldoses"])])
# merged_omics$Amino_sugars <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Amino sugars"])])
# 
# 
# merged_omics$Purines <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Purines"])])
# merged_omics$Pyrimidines <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Pyrimidines"])])
# merged_omics$Ribonucleotides <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Ribonucleotides"])])
# 
# 
# merged_omics$Alkaloids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_1 == "Alkaloids"])])
# merged_omics$Alkaloids_lysine <- (merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Alkaloids derived from lysine"])])
# merged_omics$Alkaloids_nicotinic_acid <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Alkaloids derived from nicotinic acid"])])
# 
# merged_omics$Fatty_Acids_Conjugates <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Fatty Acids and Conjugates"])])
# merged_omics$Fatty_esters <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Fatty esters"])])
# merged_omics$Flavonoids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Flavonoids"])])
# merged_omics$Isoprenoids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Isoprenoids"])])
# merged_omics$Sphingoid_bases <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Sphingoid bases"])])
# merged_omics$Steroids <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Steroids"])])
# 
# merged_omics$Biogenic_amines  <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Biogenic amines"])])
# merged_omics$Amino_acids   <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Amino acids"])])
# merged_omics$Dipeptides   <- rowMeans(merged_omics[,colnames(merged_omics) %in% c(data_hilic$Met_id[data_hilic$Subclass_2 == "Dipeptides"])])
...
jpeg("C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/Figures and data for paper/hilic_pos_all_met.jpeg", units="in", width=12, height=12, res=600)
...
write.table(Pfoa_7[,!(colnames(Pfoa_7) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/PFOA/pfoa_at_age_0_metaomics_at_7.txt", sep = "\t",
            row.names = FALSE)
...
write.table(Pfoa_14[,!(colnames(Pfoa_14) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/PFOA/pfoa_at_age_0_metaomics_at_14.txt", sep = "\t",
            row.names = FALSE)
...
write.table(Pfoa_22[,!(colnames(Pfoa_22) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/PFOA/pfoa_at_age_0_metaomics_at_22.txt", sep = "\t",
            row.names = FALSE)
...
write.table(Pfoa_28[,!(colnames(Pfoa_28) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/PFOA/pfoa_at_age_0_metaomics_at_28.txt", sep = "\t",
            row.names = FALSE)
...
write.table(pfna_7[,!(colnames(pfna_7) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/pfna/pfna_at_age_0_metaomics_at_7.txt", sep = "\t",
            row.names = FALSE)
...
write.table(pfna_14[,!(colnames(pfna_14) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/pfna/pfna_at_age_0_metaomics_at_14.txt", sep = "\t",
            row.names = FALSE)
...
write.table(pfna_22[,!(colnames(pfna_22) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/pfna/pfna_at_age_0_metaomics_at_22.txt", sep = "\t",
            row.names = FALSE)
...
write.table(pfna_28[,!(colnames(pfna_28) %in% c("chem_seq"))], file = "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/Faroese data/PFAS at age 0/pfna/pfna_at_age_0_metaomics_at_28.txt", sep = "\t",
            row.names = FALSE)
