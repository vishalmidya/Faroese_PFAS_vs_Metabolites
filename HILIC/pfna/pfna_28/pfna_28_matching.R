...
## matching
#---------------------------  at age 28
m.out1.pfna28_age28 <- matchit(cpfna28 ~ sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age7, 
                               data = merged_omics[merged_omics$Year == 28,], discard = "both", method = "full", 
                               distance = "glm", caliper = 0.5)

summary(m.out1.pfna28_age28)
f <- love.plot(m.out1.pfna28_age28)
...
## export matched data
m.out1.pfna28_age28.matched <- match.data(m.out1.pfna28_age28)
write.csv(m.out1.pfna28_age28.matched, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/New_faroese/HILIC/pfna/pfna_28/minerva_data_pfna_28_metabolites_28/matched_data_pfna_at_28_met_at_28.csv",
          row.names = F)
