...
oper <- foreach(i=1:nrow(data.pfos.0.met_at_22), .combine='comb', .multicombine=TRUE,
...
data.pfos.0.met_at_22$beta <- unlist(oper[[1]])
data.pfos.0.met_at_22$model_pval <- unlist(oper[[2]])
observed_test_stat <- unlist(oper[[1]])


d1 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_22_v1.txt")
d2 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_22_v2.txt")
d3 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_22_v3.txt")
d4 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_22_v4.txt")


l = list(d1,d2, d3, d4)
test_stat_table <- as.data.frame(data.table::rbindlist(l, use.names=TRUE))
# write.table(test_stat_table,"/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_pfos_0_met_22.txt", row.names = FALSE)


observed_test_stat <- data.pfos.0.met_at_22$beta
p_values <- foreach(p = 1:dim(data.pfos.0.met_at_22)[1], .combine = 'c') %dopar% {
  mean(abs(test_stat_table[,p]) >= abs(observed_test_stat[p]))
}
data.pfos.0.met_at_22$simu_pval <- p_values
...
write.table(wrt, "/sc/arion/work/midyav01/faroese/pfos/age0/minp_pfos_0_met_22.txt", row.names = FALSE)


# calculate the proportion of min_p_nrep that is sm/eq. p_value (for obs.)
data.pfos.0.met_at_22$adj_pval <- foreach(i = 1:length(p_values), .combine = 'c') %dopar% {
  mean(min_p_nrep <= p_values[i])
}


write.table(data.pfos.0.met_at_22,"/sc/arion/work/midyav01/faroese/pfos/age0/2e5_mwas_pfos_0_met_22.txt", row.names = FALSE)
...
# d <- read.table("/sc/arion/work/midyav01/faroese/pfos/age0/15e4_mwas_pfos_0_met_22.txt", header = T)
