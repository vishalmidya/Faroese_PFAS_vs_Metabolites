...
d1 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_28_v1.txt")
d2 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_28_v2.txt")
d3 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_28_v3.txt")
d4 <- fread("/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_table_pfos_0_met_28_v4.txt")


l = list(d1,d2, d3, d4)
test_stat_table <- as.data.frame(data.table::rbindlist(l, use.names=TRUE))
# write.table(test_stat_table,"/sc/arion/work/midyav01/faroese/pfos/age0/test_stat_pfos_0_met_28.txt", row.names = FALSE)


observed_test_stat <- data.pfos.0.met_at_28$beta
...
write.table(wrt, "/sc/arion/work/midyav01/faroese/pfos/age0/minp_pfos_0_met_28.txt", row.names = FALSE)


# calculate the proportion of min_p_nrep that is sm/eq. p_value (for obs.)
data.pfos.0.met_at_28$adj_pval <- foreach(i = 1:length(p_values), .combine = 'c') %dopar% {
  mean(min_p_nrep <= p_values[i])
}


write.table(data.pfos.0.met_at_28,"/sc/arion/work/midyav01/faroese/pfos/age0/2e5_mwas_pfos_0_met_28.txt", row.names = FALSE)
...
# d <- read.table("/sc/arion/work/midyav01/faroese/pfos/age0/15e4_mwas_pfos_0_met_28.txt", header = T)
