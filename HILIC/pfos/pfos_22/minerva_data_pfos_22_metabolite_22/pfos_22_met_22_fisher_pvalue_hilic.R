...
m.out1.pfos_22_age22.matched <- read.csv("/sc/arion/work/yaom03/new_faroese/hilic/pfos/pfos_22/minerva_data_pfos_22_metabolites_22/matched_data_pfos_at_22_met_at_22.csv")

data = m.out1.pfos_22_age22.matched[,c(paste0("Met",seq(1:nrow(data_hilic))), 'cpfos_22', 'sex',
                                    'mage',  'mbmi', 'smokepreg_2', 'cmatfishpreg', 'cparity', 'age22' )]

data.pfos_22.met_at_22 <- cbind(data_hilic[,c("mz","time","KEGG","Annotation.confidence.score","chem_name","Met_id")])
...
oper <- foreach(i=1:nrow(data.pfos_22.met_at_22), .combine='comb', .multicombine=TRUE,
                .init=list(list(), list())) %dopar% {
                  ...
                  model <- (lm(data[,i] ~ cpfos_22 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age22, data = data))
                  s <- summary(model)
...
system.time(test_stat <- foreach(i=1:nrow(data.pfos_22.met_at_22), .combine='rbind', .multicombine=TRUE) %:% 
              foreach(j = 1:iterations, .combine = 'c') %dopar% {
                f_perm <- lm(sample(data_permuted[,i]) ~ cpfos_22 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age22, data=data_permuted)
...
write.table(test_stat_table,"/sc/arion/work/yaom03/new_faroese/hilic/pfos/pfos_22/minerva_data_pfos_22_metabolites_22/pfos_22_met_22_hypothetical_test_stat_hilic.txt", row.names = FALSE)
...
write.table(data.pfos_22.met_at_22,"/sc/arion/work/yaom03/new_faroese/hilic/pfos/pfos_22/minerva_data_pfos_22_metabolites_22/pfos_22_met_22_beta_fisher_hilic.txt", row.names = FALSE)
