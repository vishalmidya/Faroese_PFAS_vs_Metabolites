...
m.out1.pfhxs_0_age7.matched <- read.csv("/sc/arion/work/yaom03/new_faroese/hilic/pfhxs/pfhxs_0/minerva_data_pfhxs_0_metabolites_7/matched_data_pfhxs_at_0_met_at_7.csv")

data = m.out1.pfhxs_0_age7.matched[,c(paste0("Met",seq(1:nrow(data_hilic))), 'cpfhxs_0', 'sex',
                                    'mage',  'mbmi', 'smokepreg_2', 'cmatfishpreg', 'cparity', 'age7' )]

data.pfhxs_0.met_at_7 <- cbind(data_hilic[,c("mz","time","KEGG","Annotation.confidence.score","chem_name","Met_id")])
...
oper <- foreach(i=1:nrow(data.pfhxs_0.met_at_7), .combine='comb', .multicombine=TRUE,
                .init=list(list(), list())) %dopar% {
                  ...
                  model <- (lm(data[,i] ~ cpfhxs_0 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age7, data = data))
                  s <- summary(model)
...
system.time(test_stat <- foreach(i=1:nrow(data.pfhxs_0.met_at_7), .combine='rbind', .multicombine=TRUE) %:% 
              foreach(j = 1:iterations, .combine = 'c') %dopar% {
                f_perm <- lm(sample(data_permuted[,i]) ~ cpfhxs_0 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age7, data=data_permuted)
...
write.table(test_stat_table,"/sc/arion/work/yaom03/new_faroese/hilic/pfhxs/pfhxs_0/minerva_data_pfhxs_0_metabolites_7/pfhxs_0_met_7_hypothetical_test_stat_hilic.txt", row.names = FALSE)
...
write.table(data.pfhxs_0.met_at_7,"/sc/arion/work/yaom03/new_faroese/hilic/pfhxs/pfhxs_0/minerva_data_pfhxs_0_metabolites_7/pfhxs_0_met_7_beta_fisher_hilic.txt", row.names = FALSE)
