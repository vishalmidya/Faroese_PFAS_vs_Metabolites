...
m.out1.pfda_0_age22.matched <- read.csv("/sc/arion/work/yaom03/new_faroese/hilic/pfda/pfda_0/minerva_data_pfda_0_metabolites_22/matched_data_pfda_at_0_met_at_22.csv")

data = m.out1.pfda_0_age22.matched[,c(paste0("Met",seq(1:nrow(data_hilic))), 'cpfda_0', 'sex',
                                    'mage',  'mbmi', 'smokepreg_2', 'cmatfishpreg', 'cparity', 'age22' )]

data.pfda_0.met_at_22 <- cbind(data_hilic[,c("mz","time","KEGG","Annotation.confidence.score","chem_name","Met_id")])
...
oper <- foreach(i=1:nrow(data.pfda_0.met_at_22), .combine='comb', .multicombine=TRUE,
                .init=list(list(), list())) %dopar% {
                  ...
                  model <- (lm(data[,i] ~ cpfda_0 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age22, data = data))
                  s <- summary(model)
...
system.time(test_stat <- foreach(i=1:nrow(data.pfda_0.met_at_22), .combine='rbind', .multicombine=TRUE) %:% 
              foreach(j = 1:iterations, .combine = 'c') %dopar% {
                f_perm <- lm(sample(data_permuted[,i]) ~ cpfda_0 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age22, data=data_permuted)
...
write.table(test_stat_table,"/sc/arion/work/yaom03/new_faroese/hilic/pfda/pfda_0/minerva_data_pfda_0_metabolites_22/pfda_0_met_22_hypothetical_test_stat_hilic.txt", row.names = FALSE)
...
write.table(data.pfda_0.met_at_22,"/sc/arion/work/yaom03/new_faroese/hilic/pfda/pfda_0/minerva_data_pfda_0_metabolites_22/pfda_0_met_22_beta_fisher_hilic.txt", row.names = FALSE)
