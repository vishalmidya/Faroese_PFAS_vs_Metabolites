...
m.out1.pfda_22_age28.matched <- read.csv("/sc/arion/work/yaom03/new_faroese/hilic/pfda/pfda_22/minerva_data_pfda_22_metabolites_28/matched_data_pfda_at_22_met_at_28.csv")

data = m.out1.pfda_22_age28.matched[,c(paste0("Met",seq(1:nrow(data_hilic))), 'cpfda_22', 'sex',
                                    'mage',  'mbmi', 'smokepreg_2', 'cmatfishpreg', 'cparity', 'age28' )]

data.pfda_22.met_at_28 <- cbind(data_hilic[,c("mz","time","KEGG","Annotation.confidence.score","chem_name","Met_id")])
...
oper <- foreach(i=1:nrow(data.pfda_22.met_at_28), .combine='comb', .multicombine=TRUE,
                .init=list(list(), list())) %dopar% {
                  ...
                  model <- (lm(data[,i] ~ cpfda_22 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age28, data = data))
                  s <- summary(model)
...
system.time(test_stat <- foreach(i=1:nrow(data.pfda_22.met_at_28), .combine='rbind', .multicombine=TRUE) %:% 
              foreach(j = 1:iterations, .combine = 'c') %dopar% {
                f_perm <- lm(sample(data_permuted[,i]) ~ cpfda_22 + sex + mage + mbmi  + smokepreg_2 + cmatfishpreg  + cparity + age28, data=data_permuted)
...
write.table(test_stat_table,"/sc/arion/work/yaom03/new_faroese/hilic/pfda/pfda_22/minerva_data_pfda_22_metabolites_28/pfda_22_met_28_hypothetical_test_stat_hilic.txt", row.names = FALSE)
...
write.table(data.pfda_22.met_at_28,"/sc/arion/work/yaom03/new_faroese/hilic/pfda/pfda_22/minerva_data_pfda_22_metabolites_28/pfda_22_met_28_beta_fisher_hilic.txt", row.names = FALSE)
