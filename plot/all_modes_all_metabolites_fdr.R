...
pfas_met_tab <- pfas_met_tab_all[pfas_met_tab_all$fdr < 0.05 & pfas_met_tab_all$PFAS_age == "0",]
dim(pfas_met_tab)
...
pfas_met_tab <- pfas_met_tab_all[pfas_met_tab_all$fdr < 0.05 & pfas_met_tab_all$PFAS_age == "7",]
dim(pfas_met_tab)
...
pfas_met_tab <- pfas_met_tab_all[pfas_met_tab_all$fdr < 0.05 & pfas_met_tab_all$PFAS_age == "14",]
dim(pfas_met_tab)
...
pfas_met_tab <- pfas_met_tab_all[pfas_met_tab_all$fdr < 0.05 & pfas_met_tab_all$PFAS_age == "22",]
dim(pfas_met_tab)
...
all_sig_hits <- all_sig_hits[,c("Metabolite","mz","time","beta","PFAS","Age","PFAS_age","Mode","Met_id","simu_pval","fdr")]


write.csv(all_sig_hits, "C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/New_faroese/sig_metabolites_fdr_closest.csv", row.names = F )
...
all_sig_hits <- read.csv("C:/Users/yaom03/OneDrive - The Mount Sinai Hospital/New_faroese/sig_metabolites_fdr_closest.csv")
...
vol <- (ggplot(all_sig_hits[all_sig_hits$PFAS_age == 22,], aes(x=Age, y=beta, color=PFAS, label=Metabolite, shape = Mode)) +# Show all points
        ...
        +  geom_label_repel(size = 6, family = 'serif',
                            ...
                            box.padding = unit(0.5, "lines"),
                            max.iter = 2e4,
                            max.overlaps = getOption("ggrepel.max.overlaps", default = 400),
                            force = 2, force_pull = 2, show.legend = F))
